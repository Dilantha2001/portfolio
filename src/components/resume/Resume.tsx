          ...
          {personal.headline && (
            <div className="flex flex-wrap gap-2 mt-1 print:mt-0">
              {personal.headline.split("·").map((h, i) => (
                <span
                  key={i}
                  className="px-2 py-0.5 rounded bg-slate-900/60 border border-slate-800 text-[10px] text-slate-400 font-mono print:bg-transparent print:border-none print:text-slate-600 print:p-0 print:after:content-['•'] last:print:after:content-none print:after:mx-1.5"
                >
                  {h.trim()}
                </span>
              ))}
                ...
                {contact.socials.map((s) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-slate-500 truncate group-hover:text-slate-400 transition print:text-slate-600">
                        {s.url.replace(/(^\w+:|^)\/\//, "")}
                      </div>
                    </div>
                  </a>
            ...
            <div className="bg-[#020617]/40 backdrop-blur-xl border border-slate-800/60 p-5 rounded-2xl print:bg-transparent print:border-none print:p-0">
              ...
              <div className="space-y-4">
                {resumeInfo.extras.languages && (
                  <div>
                    <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2 print:text-slate-700">Languages</h3>
                    <div className="flex flex-col gap-1.5">
                      {resumeInfo.extras.languages.map((lang) => (
                        <div key={lang.name} className="flex justify-between items-center text-xs">
                          ...
                          <span className="text-[10px] text-slate-500 font-medium print:text-slate-600">{lang.level}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {resumeInfo.extras.interests && (
                  <div>
                    <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2 print:text-slate-700">Interests</h3>
                    <div className="flex flex-wrap gap-1.5">
                      {resumeInfo.extras.interests.map((interest) => (
                        <span
                          key={interest}
                          className="px-2 py-0.5 rounded bg-slate-900/60 border border-slate-850 text-[10px] text-slate-400 print:bg-transparent print:border-none print:text-slate-800 print:p-0 print:after:content-[','] last:print:after:content-none print:after:mr-1"
                        >
                          {interest}
                        </span>
                      ))}
              ...
              <div className="space-y-6">
                {resumeInfo.experience.map((exp, idx) => (
                  ...
                  >
                    ...
                    {exp.summary && (
                      <p className="mt-2.5 text-xs text-slate-400 leading-relaxed text-justify print:text-slate-800">
                        {exp.summary}
                      </p>
                    )}

                    {exp.bullets && exp.bullets.length > 0 && (
                      <ul className="list-disc list-outside mt-3 ml-4 space-y-1.5 text-xs text-slate-400 leading-normal print:text-slate-800 print:mt-1">
                        {exp.bullets.map((b, i) => (
                    ...
                    {exp.tech && exp.tech.length > 0 && (
                      <div className="mt-4 flex flex-wrap gap-1.5 items-center print:mt-1">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider font-mono mr-1 print:text-slate-600">Stack:</span>
                        {exp.tech.map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded bg-slate-950/60 border border-slate-900 text-[10px] text-slate-400 font-mono print:bg-transparent print:border-none print:text-slate-800 print:p-0 print:after:content-[','] last:print:after:content-none print:after:mr-1"
                          >
                            {t}
                          </span>
                        ))}
                ...
                {resumeInfo.projects.slice(0, 4).map((p) => (
                  ...
                  >
                    <div>
                      ...
                      {p.description && (
                        <p className="mt-2 text-[11px] text-slate-400 leading-relaxed text-justify print:text-slate-800 print:mt-0.5">
                          {p.description}
                        </p>
                      )}
                    </div>
                    {p.tags && p.tags.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1 print:mt-1">
                        {p.tags.slice(0, 4).map((t) => (
                          <span
                            key={t}
                            className="px-1.5 py-0.5 rounded bg-slate-900/60 border border-slate-850 text-[9px] text-slate-400 font-mono print:bg-transparent print:border-none print:text-slate-800 print:p-0 print:after:content-[','] last:print:after:content-none print:after:mr-1"
                          >
                            {t}
                          </span>
                        ))}
